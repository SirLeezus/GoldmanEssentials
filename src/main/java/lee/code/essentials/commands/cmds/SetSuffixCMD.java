    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, String[] args) {
        ...
        if (args.length > 1) {
            ...
            if (target != null) {
                ...
                String suffix = " " + pu.buildStringFromArgs(args, 1);
                cache.setSuffix(uuid, suffix);
                if (target.isOnline()) {
                    Player tPlayer = target.getPlayer();
                    if (tPlayer != null) pu.updateDisplayName(tPlayer, false);
                }
                sender.sendMessage(Lang.PREFIX.getComponent(null).append(Lang.COMMAND_SETSUFFIX_SUCCESSFUL.getComponent(new String[]{ target.getName(), pu.format(suffix) })));
