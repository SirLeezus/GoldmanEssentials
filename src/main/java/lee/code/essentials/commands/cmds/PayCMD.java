                    ...
                    if (valueScanner.hasNextLong()) {
                        ...
                        if (!targetUUID.equals(uuid)) {
                            if (senderBalance >= payAmount) {
                                ...
                                player.sendMessage(Lang.PREFIX.getComponent(null).append(Lang.COMMAND_PAY_SENDER_SUCCESSFUL.getComponent(new String[] { target.getName(), plugin.getPU().formatAmount(payAmount) })));
                                if (target.isOnline()) {
                                    Player oTarget = target.getPlayer();
                                    if (oTarget != null) oTarget.sendMessage(Lang.PREFIX.getComponent(null).append(Lang.COMMAND_PAY_TARGET_SUCCESSFUL.getComponent(new String[] { plugin.getPU().formatAmount(payAmount), player.getName() })));
                                }
                            } else player.sendMessage(Lang.PREFIX.getComponent(null).append(Lang.ERROR_COMMAND_PAY_INSUFFICIENT_FUNDS.getComponent(new String[] { target.getName(), plugin.getPU().formatAmount(payAmount), plugin.getPU().formatAmount(senderBalance) })));
