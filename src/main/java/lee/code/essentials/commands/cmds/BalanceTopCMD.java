...
public class BalanceTopCMD implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, String[] args) {
        ...
        if (sender instanceof Player player) {
            ...
            Map<UUID, Long> playerBalances = cacheManager.getTopBalances();
            HashMap<UUID, Long> sortedMap = pu.sortByLong(playerBalances);
            ...
            for (int i = 0; i < maxDisplayed; i++) {
                ...
                if (players.get(index) != null) {
                    ...
                    if (offlinePlayer.getName() != null) {
                        ...
                        String balance = BukkitUtils.parseValue(sortedMap.get(pUUID));
                        if (name.equals(player.getName())) {
                            posColor = "&2";
                            onPage = true;
                        }
                        char nameColor = cacheManager.getColor(pUUID).getChar();
                        lines.add(BukkitUtils.parseColorComponent(posColor + position + ". &" + nameColor + name + " &7| &6$" + balance));
                        position++;
                    }
                }
            }
            ...
            if (!onPage) {
                lines.add(Component.text(""));
                lines.add(BukkitUtils.parseColorComponent("&2" + (players.indexOf(uuid) + 1) + ". &" + cacheManager.getColor(uuid).getChar() + player.getName() + " &7| &6$" + BukkitUtils.parseValue(sortedMap.get(uuid))));
            }
