    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, String[] args) {
        ...
        if (args.length > 1) {
            ...
            if (target != null) {
                ...
                String prefix = pu.buildStringFromArgs(args, 1) + " ";
                cache.setPrefix(uuid, prefix);
                if (target.isOnline()) {
                    Player tPlayer = target.getPlayer();
                    if (tPlayer != null) pu.updateDisplayName(tPlayer, false);
                }
                sender.sendMessage(Lang.PREFIX.getComponent(null).append(Lang.COMMAND_SETPREFIX_SUCCESSFUL.getComponent(new String[]{ target.getName(), pu.format(prefix) })));
